...
An inserted value may be generated by a registered template expression:
...
`date.now()` resolves at scaffold-apply time to the current local date in `YYYY-MM-DD` format.
Generated inserts initialize empty fields only and never overwrite or append to existing values.
...
All modular constructor files and configured JSON/YAML schema files are linted before entering the ontology graph.
